...
let flag = true
if (flag) {
    document.write('<h1>漫漫愁云起，苍苍别路迷。</h1>')
}
```
...
let flag = false
if (flag) {
    document.write('<h1>泠泠七弦上，静听松风寒。</h1>')
} else {
    document.write('<h1>古调虽自爱，今人多不弹。</h1>')
}
```
...
if (year % 4 === 0 && year % 100 !== 0 || year % 400 === 0) {
    document.write(`<h1>${year}年是闰年</h1>`)
} else {
    document.write(`<h1>${year}年是平年</h1>`)
}
```
...
三元运算符：比`if`双分支更简单的写法，有时候也叫做三元表达式（条件表达式），三元运算最后又一个返回值。
