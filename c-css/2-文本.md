...
* 属性关键词：CSS中属性关键词指定了样式的预设值，不同的属性关键词不尽相同。
...
1. px（像素）：CSS常用单位，是一个绝对单位。在任何显示器中，相同像素值用于显示的点数是一样的。
2. 谷歌浏览器默认文字大小是16px。
3. 字体大小设置处像素外还有：`em`、`rem`等单位。

#### 像素的概念

像素：是数字图像中最小的独立控制单位。每个像素在图像中通常以一个小方块的形式存在，并包含一个特定的颜色或灰度值。

不同的显示器屏幕中像素的密度有所不同。

<img src="https://raw.githubusercontent.com/hughxusu/lesson-web/developing/_images/c-css/webp.jpg" style="zoom:80%;" />

#### 样式中的颜色

颜色是CSS中的常用属性，不止于字体上，给CSS的属性设置颜色值有多重方式。

| 设置颜色的方式 | 含义                                | 属性值                                              | 说明                     |
| -------------- | ----------------------------------- | --------------------------------------------------- | ------------------------ |
| 关键词         | 预定义颜色名称                      | `red`、`blue`                                       | 只包含常见英文单词。     |
...
1. 如果字体名称中存在多个单词，推荐使用引号包裹，如：`"Times New Roman"`。
...
> * 或者使用大语言模型查询[文心一言](https://yiyan.baidu.com/?)
...
控制行的上下行间距，可以使用像素和倍数两种值（**通常使用倍数值**）。

<img src="https://raw.githubusercontent.com/hughxusu/lesson-web/developing/_images/c-css/pp9ZeQx.png" style="zoom:35%;" />
...
* [原文链接](https://hanyu.baidu.com/shici/detail?from=aladdin&pid=819312a1b1cf517e82938dff0c7ed7a4)
* [作者介绍](https://baike.baidu.com/link?url=Khgmtq0wKzT_7PVsIn5K2JCqbN2VQ7Uke3ap9-gggxFyl59LZAtHNe4rz2PNnyMakaMmwP5SbO6JiZDiQW_Wd_ji6ZjDJDSSG88XDT2Mx5O)

<img src="https://raw.githubusercontent.com/hughxusu/lesson-web/developing/_images/c-css/Xnip2024-08-09_16-27-20.jpg" style="zoom: 45%;" />
